...
$(document).ready(function () {
    ...
    $("#thumb article img").hover(
        function () {
            $(this).parent('a').parent('figure').children('figcaption').fadeOut();
        }, function () {
            $(this).parent('a').parent('figure').children('figcaption').fadeIn();
        }
    );
    /*
...
var slideCount = $('#slider-productos ul li').length;
var slideWidth = $('#slider-productos ul li').width();
var slideHeight = $('#slider-productos ul li').height();
var sliderUlWidth = slideCount * slideWidth + 80; // (10*4)*2 where 10 is margin between li elements.

$('#slider-productos').css({width: sliderUlWidth, height: slideHeight});

$('#slider-productos ul').css({width: sliderUlWidth /*, marginLeft: - slideWidth */});

$('#slider-productos ul li:last-child').prependTo('#slider-productos ul');

function moveLeft() {
    $('#slider-productos ul').animate({
        left: +slideWidth
    }, 2000, function () {
        $('#slider-productos ul li:last-child').prependTo('#slider-productos ul');
        $('#slider-productos ul').css('left', '');
    });
};
...
function moveRight() {
    $('#slider-productos ul').animate({
        ...
        $('#slider-productos ul li:first-child').appendTo('#slider-productos ul');
        $('#slider-productos ul').css('left', '');
    });
};
...
$("#slider-productos ul li").on("swipeleft", function () {
    //alert("You swiped left!");
    moveRight();
});

$("#slider-productos ul li").on("swiperight", function () {
    //alert("You swiped left!");
    moveLeft();
});
